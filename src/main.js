...
function init() {
    createInitWindow(`file:///${__dirname}/public/index.html`);
...
function createInitWindow(url) {
  ...
  if (isDev) initWin.webContents.openDevTools();
    ...
    //prevent cache
          ...
          switch (checkURL(url)) {
            case "social":
              ...
              break;
            default:
              initWin.loadURL(`file:///${__dirname}/public/index.html`);
              break;
          }
    });
  ...
  shortcut.register(initWin, "F3", () => {
      ...
      initWin.webContents.executeJavaScript('pc.app.fire("Chat:Message", "Seven-Network", "Link copied!")').catch(e=>{});
  })
  ...
  initWin.webContents.on("dom-ready", (event) => {
    initWin.setTitle(`Seven-Network}`);
    event.preventDefault();
  });
  ...
  function LinkBox() {
    ...
    function isPaste(message, isSpectate) {
      ...
      if (isSpectate) {
        initWin.loadURL(`${__dirname}/public/index.html#Spectate:${inviteCode}`);
      } else {
        initWin.loadURL(`${__dirname}/public/index.html#${inviteCode}`);
      }
    }
...
autoUpdater.on('update-available', (info) => {
    const dialogOpts = {
        type: 'info',
        buttons: ['Alright!'],
        title: 'Seven-Network has been updated',
        message: 'Due to latest changes in our Network. We have to update your client accordingly to it.',
        detail: 'If nothing happens after 5 minutes after pressing OK, please contact us in our discord for support'
       }
...
autoUpdater.on('update-downloaded', (event, releaseNotes, releaseName) => {
    const dialogOpts = {
      ...
      detail: 'Update for the client has been updated. Please Restart the client'
     }
